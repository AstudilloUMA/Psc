...
public class mergesort {
	...
	private static List<Integer> crear_lista_aleatoria() {
		...
		for(int i = 0; i < 100; i++) lista.add(rnd.nextInt(100));
...
class Nodo extends Thread {
	...
	private void mezcla(List<Integer> l1, List<Integer> l2) {
		...
		if(i1 == l1.size()) l.addAll(l2.subList(i2,l2.size()));
		else l.addAll(l1.subList(i1,l1.size()));
	}
	...
	public void run() {
		try {
			...
			if (sz > 1) {
				...
				l1.addAll(l.subList(0,l.size()/2));
