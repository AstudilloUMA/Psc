...
void mostrar(TLista lista)
{
    while (lista != NULL)
    {
        printf("Numero de Frame: %d\t Nombre: %s\n", lista->nFrame, lista->nombre);
        lista = lista->sig;
    }
}
...
void insertar(TLista *lista, unsigned int pos, char *nombre)
{
    ...
    TLista nuevoNodo = (TLista)malloc(sizeof(struct TNodo));
    if(nuevoNodo == NULL) perror("insertar-> error al reservar memoria");
    else
    {
        nuevoNodo->nFrame = pos;
        ...
        {
            while (actual != NULL && actual->nFrame < pos && actual->nFrame != pos)
            {
                prev = actual;
                actual = actual->sig;
            }
...
void crearDesdeFichero(TLista *lista, FILE *fich)
{
    ...
    while (fscanf(fich,"%u %s",&frame,nombre))
    {
    ...
    if(salto > 1)
    {
        if(*lista != NULL)
        {
            ...
            while (actual != NULL)
            {
                ...
                {
                    (*numFrames)++;
                    fprintf(eliminados, "%d %s", actual->nFrame, actual->nombre);
                    prev->sig = actual->sig;
                    free(actual);
                    actual = prev->sig;
                    cont = 0;
...
int eliminar(TLista *lista, unsigned int *numFrame, char *nombre)
{
    ...
    {
        *numFrame = nodo->nFrame;
        strcpy(nombre, nodo->nombre);
        *lista = (*lista)->sig;
        free(nodo);
        return 1;
    }
